...
FROM python:3.9-slim AS pip
RUN apt-get update && apt-get install -y gcc
RUN pip install --no-cache-dir psutil

# Stage bess: creates the runtime image of BESS
FROM python:3.9-slim AS bess
RUN apt-get update && \
    ...
    pip install --no-cache-dir \
        ...
        pyroute2 \
        scapy
COPY --from=pip /usr/local/lib/python3.9/site-packages/psutil /usr/local/lib/python3.9/site-packages/psutil
