...
class Port:
    ...
    def init_port(self, idx, conf_mode):
        ...
        self.num_q = num_q
        print("Setting up port {} on worker ids {}".format(self.name, self.workers))

        if conf_mode not in ["dpdk", "sim"]:
            raise Exception("Invalid mode: {} selected.".format(conf_mode))

        if conf_mode == "sim":
            self.fpi = Source(name="{}_source".format(self.name))
            self.fpo = Sink(name="{}_out".format(self.name))
            self.bpf = BPF(name="{}FastBPF".format(self.name))
            self.bpf.clear()
        ...
        if conf_mode == "dpdk":
            # Fallback to scanning ports if port list is empty, scan for dpdk_ports first
            ...
            if idx is None:
                raise Exception(
                    "Registered port for {} not detected!".format(self.name)
                )
        ...
        try:
            name = self.veth_name.replace(
                "-", "_"
            )  # BESS web UI chokes on dashes in module names.
            peer = peer_by_interface(self.veth_name)
            ...
            print("Mirror veth interface: {} misconfigured: {}".format(veth_name, e))
            raise e
            ...
            fields=[
                {
                    "offset": 6,
                    "size": 6,
                    "value": mac2hex(mac_by_interface(self.veth_name)),
                }
            ],
        )
