...
int
Sch::cfg_load_pipe(struct rte_cfgfile *cfg, struct rte_sched_pipe_params *pipe_params)
{
	int i, j;
	char *next;
	const char *entry;
...
int Sch::cfg_load_subport_profile(struct rte_cfgfile *cfg,struct rte_sched_subport_profile_params *subport_profile)
{
	int i;
	const char *entry;
	int profiles;
...
int Sch::cfg_load_qfi_profile(struct rte_cfgfile *cfg)
{
	...
	if(entry)
       ...
       for(uint32_t i=1;i<=qfi_num;i++)
	   {
		...
		for (i = 0; i < 5; i++) 
		 {
		        arr[i] = (uint8_t)strtol(entry, &next, 10);
				if (next == NULL)
					break;
				entry = next;
		 }
 qfi[arr[i]][1][1][1][1] = arr[0]; qfi[arr[i]][0][1][1][1] =arr[1]; qfi[arr[i]][1][0][1][1] =arr[2]; qfi[arr[i]][1][1][0][1]=arr[3];
 qfi[arr[i]][1][1][1][0] = arr[4];
	   }
    return 1;
}
