 ...
 */
bess::Packet *
IPDefrag::IPReassemble(Context *ctx, bess::Packet *p)
{
	Ethernet *eth = p->head_data<Ethernet *>();
	if (eth->ether_type != (be16_t)(Ethernet::kIpv4))	return p;
...
/*----------------------------------------------------------------------------------*/
void
IPDefrag::ProcessBatch(Context *ctx, bess::PacketBatch *batch)
{
...
/*----------------------------------------------------------------------------------*/
CommandResponse
IPDefrag::Init(const bess::pb::IPDefragArg &arg) {
