...
// msgTableFull is the message that accompanies it, shared so that goconst does not count
// the third use of the literal against the two that came before.
const msgTableFull = "table is full"
...
func TestSendMsgToUPFRejectsARuleTheModuleRefused(t *testing.T) {
	refused := &pb.CommandResponse{Error: &pb.Error{Code: errnoNoSpace, Errmsg: msgTableFull}}
...
// answers ENOENT for a rule it does not hold, WildcardMatch for a mask it has no rule
// under.
func TestSendMsgToUPFAcceptsADeleteOfARuleTheDatapathDoesNotHold(t *testing.T) {
	b := answeringBess(&pb.CommandResponse{
		Error: &pb.Error{Code: errnoNoEntry, Errmsg: "rule doesn't exist"},
	}, nil)
...
// The exception above is confined to deletes for a reason: bessctl answers ENOENT for a
// module that is not in the graph at all, so if an add were excepted too a mistyped or
// missing pdrLookup would read as a datapath that had accepted every rule. An add reports
// it, which is what makes a missing module impossible to hide.
func TestSendMsgToUPFRejectsAnAddAnsweredWithNoEntry(t *testing.T) {
	b := answeringBess(&pb.CommandResponse{
		Error: &pb.Error{Code: errnoNoEntry, Errmsg: "No module 'pdrLookup' found"},
	}, nil)

	rules := PacketForwardingRules{pdrs: []pdr{{}}}

	if cause := b.SendMsgToUPF(upfMsgTypeAdd, rules, PacketForwardingRules{}); cause != ie.CauseRequestRejected {
		t.Errorf("SendMsgToUPF() = %d for an add answered ENOENT, want CauseRequestRejected (%d); "+
			"the delete exception must not reach the add path",
			cause, ie.CauseRequestRejected)
	}
}

// An RPC that did not complete says nothing about what the datapath did with the rule,
// and must not be reported as a refusal. SendMsgToUPF answers a batch that did not
// complete as accepted so that an uncertain result does not have the caller tear down a
// session the datapath may still hold -- and a rule reported as refused would reach the
// opposite answer.
//
// This is the common shape, not a corner: the context these calls carry is created before
// GRPCJoin's timer starts, so a slow datapath expires the context first and every worker
// sees DeadlineExceeded while the join itself never times out.
func TestSendMsgToUPFDoesNotRejectARuleWhoseRPCDidNotComplete(t *testing.T) {
	for _, tc := range []struct {
		name string
		err  error
	}{
		{"deadline exceeded", status.Error(codes.DeadlineExceeded, "context deadline exceeded")},
		{"connection refused", status.Error(codes.Unavailable, "connection refused")},
		{"entry not found", status.Error(codes.NotFound, "entry not found")},
	} {
		t.Run(tc.name, func(t *testing.T) {
			b := answeringBess(nil, tc.err)

			rules := PacketForwardingRules{pdrs: []pdr{{}}, fars: []far{{}}, qers: []qer{{}}}

			if cause := b.SendMsgToUPF(upfMsgTypeAdd, rules, PacketForwardingRules{}); cause != ie.CauseRequestAccepted {
				t.Errorf("SendMsgToUPF() = %d for a batch whose RPCs did not complete, want CauseRequestAccepted (%d); "+
					"nothing here knows what the datapath did, and rejecting makes the caller discard a session it may hold",
					cause, ie.CauseRequestAccepted)
			}
		})
	}
...
// AddSliceInfo is the only caller of a datapath worker that is not SendMsgToUPF, and it
// returned nil whatever the join said -- so the slice-meter reporting this change adds
// would have been discarded one layer up, which is the shape that made the first attempt
// at this chain ineffective. It reports on the same condition SendMsgToUPF uses, and for
// the same reason it must not report an incomplete batch.
func TestAddSliceInfoReportsAMeterTheDatapathRefused(t *testing.T) {
	b := answeringBess(&pb.CommandResponse{
		Error: &pb.Error{Code: errnoNoSpace, Errmsg: msgTableFull},
	}, nil)

	if err := b.AddSliceInfo(&SliceInfo{uplinkMbr: 100000, downlinkMbr: 100000}); err == nil {
		t.Error("AddSliceInfo() = nil for a meter the datapath refused, want an error")
	}
}

func TestAddSliceInfoDoesNotReportABatchThatDidNotComplete(t *testing.T) {
	release := make(chan struct{})
	defer close(release)
	...
	if err := b.AddSliceInfo(&SliceInfo{uplinkMbr: 100000, downlinkMbr: 100000}); err != nil {
		t.Errorf("AddSliceInfo() = %v for a batch that timed out, want nil; "+
