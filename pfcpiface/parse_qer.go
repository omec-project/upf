...
func (q *qer) parseQER(ie1 *ie.IE, seid uint64) error {
	...
	qfi, err := ie1.QFI()
	if err != nil {
		logger.PfcpLog.Errorln("could not read QFI")
	}

	gsUL, err := ie1.GateStatusUL()
	if err != nil {
		logger.PfcpLog.Errorln("could not read Gate status uplink")
	}

	gsDL, err := ie1.GateStatusDL()
	if err != nil {
		logger.PfcpLog.Errorln("could not read Gate status downlink")
	}

	mbrUL, err := ie1.MBRUL()
	if err != nil {
		logger.PfcpLog.Errorln("could not read MBRUL")
	}

	mbrDL, err := ie1.MBRDL()
	if err != nil {
		logger.PfcpLog.Errorln("could not read MBRDL")
	}
