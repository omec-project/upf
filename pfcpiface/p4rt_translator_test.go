...
//TODO replace mockP4INFO with a different P4INFO w.r.t the one used in conf/
...
func setupNewTranslator(t *testing.T) *P4rtTranslator {
	...
	err := proto.UnmarshalText(mockP4INFO, &p4Config)
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				got := translator.actionID(tt.args)
...
func TestTableID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				got := translator.tableID(tt.args)
				require.Equal(t, tt.want, got)
			},
		)
	}
