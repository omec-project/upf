...
func (b *bess) readCounterVal() {
	log.Println("bess readCounterVal not handled")
}

func (b *bess) handleVolQuotaExceed(pdrID uint32, keySeid uint64) {
	log.Println("bess handleVolQuotaExceed not handled")
}

func (b *bess) addUsageReports(
	sdRes *message.SessionDeletionResponse, seidKey uint64) {
	log.Println("bess addUsageReports not handled")
}
func (b *bess) sendURRForReporting(recItem *reportRecord) {
...
func (b *bess) handleCounterEntry(ce *IntfCounterEntry) {
	log.Println("bess handleCounterEntry not handled")
}

func (b *bess) sendMsgToUPF(method string, pdrs []pdr, fars []far, urrs []urr) uint8 {
	// create context
