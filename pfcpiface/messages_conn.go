...
func (pConn *PFCPConn) sendAssociationRequest() {
	...
	if reply != nil {
		...
		if err != nil {
			logger.PfcpLog.Errorln("handling of Assoc Setup Response Failed", pConn.RemoteAddr())
			pConn.Shutdown()
...
func (pConn *PFCPConn) handleHeartbeatRequest(msg message.Message) (message.Message, error) {
	...
	if pConn.upf.enableHBTimer {
		...
		default:
			...
			logger.PfcpLog.Warnln("failed to reset heartbeat timer")
		}
	}
...
func (pConn *PFCPConn) handleAssociationSetupRequest(msg message.Message) (message.Message, error) {
	...
	if pConn.ts.remote.IsZero() {
		pConn.ts.remote = ts
		logger.PfcpLog.Infoln("association Setup Request from", addr,
			"with recovery timestamp:", ts)
	} else if ts.After(pConn.ts.remote) {
		old := pConn.ts.remote
		pConn.ts.remote = ts
		logger.PfcpLog.Warnln("association Setup Request from", addr,
	...
	logger.PfcpLog.Infoln("association setup done between nodes",
...
func (pConn *PFCPConn) handlePFDMgmtRequest(msg message.Message) (message.Message, error) {
	...
	for _, appIDPFD := range pfdmreq.ApplicationIDsPFDs {
		...
		logger.PfcpLog.Debugf("flow descriptions for AppID %v: %v", id, applicationPFD.flowDescs)
	}
