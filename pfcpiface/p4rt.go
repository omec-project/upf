...
func channelSetup(u *upf, p *p4rtc) (*P4rtClient, error) {
	...
	if localclient != nil {
		...
		u.accessIP, p.accessIPMask, errin =
			setSwitchInfo(localclient)
		if errin != nil {
			log.Println("Switch set info failed ", errin)
			return nil, errin
		}
		log.Println("accessIP, Mask ", u.accessIP, p.accessIPMask)
...
func (p *p4rtc) handleChannelStatus(u *upf) bool {
	var errin error
	if p.p4client == nil || p.p4client.CheckStatus() != Ready {
		p.p4client, errin = channelSetup(u, p)
		if errin != nil {
			log.Println("create channel failed : ", errin)
			return true
		} else {
...
func (p *p4rtc) sendDeleteAllSessionsMsgtoUPF(u *upf) {
	...
	if (p.pfcpConn != nil) && (p.pfcpConn.mgr != nil) {
		for seidKey, value := range p.pfcpConn.mgr.sessions {
			p.sendMsgToUPF(u, "del", value.pdrs, value.fars)
...
func (p *p4rtc) sendMsgToUPF(u *upf, method string, pdrs []pdr,
	fars []far) uint8 {
