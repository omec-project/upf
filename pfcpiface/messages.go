...
func (pc *PFCPConn) handleSessionReportResponse(msg message.Message, addr net.Addr, intf common) {
...
func (pc *PFCPConn) handleSessionEstablishmentRequest(intf common, msg message.Message, addr net.Addr, sourceIP string) []byte {
	...
	recItem := reportRecord{
		fseid: session.localSEID,
		srcIP: sourceIP,
		urrs:  &session.urrs,
	}
	intf.sendURRForReporting(&recItem)
	intf.sendMsgToUPF("add", session.pdrs, session.fars, session.urrs)
...
func (pc *PFCPConn) handleSessionModificationRequest(intf common, msg message.Message, addr net.Addr, sourceIP string) []byte {
	...
	intf.sendMsgToUPF("mod", addPDRs, addFARs, nil)
	...
	intf.sendMsgToUPF("del", delPDRs, delFARs, nil)
...
func (pc *PFCPConn) handleSessionDeletionRequest(intf common, msg message.Message, addr net.Addr, sourceIP string) []byte {
	...
	intf.sendMsgToUPF("del", session.pdrs, session.fars, nil)
	...
	intf.addUsageReports(smres, localSEID)
	delRes, err := smres.Marshal()
	if err != nil {
