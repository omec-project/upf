...
func buildExpectedSessionsDownlinkEntry(client *p4rtc.Client, expected p4RtValues) *p4_v1.TableEntry {
	...
	// randomly generated by pfcpiface
	dummySessMeterIndex := []byte{0x00, 0x00}
	dummyTunnelPeerID := []byte{0x00, 0x00}

	return client.NewTableEntry(TableDownlinkSessions, []p4rtc.MatchInterface{
		&p4rtc.ExactMatch{
			Value: ueAddr,
		},
	}, client.NewTableActionDirect(ActSetDownlinkSession, [][]byte{dummyTunnelPeerID, dummySessMeterIndex}), nil)
}
...
func verifyP4RuntimeEntries(t *testing.T, testdata *pfcpSessionData, expectedValues p4RtValues, afterModification bool) {
	...
	if len(entries) > 0 {
		require.Equal(t, expectedApplicationsEntry.Match, entries[0].Match, "PreQosPipe.applications match fields does not equal expected",
			entries)
		require.Equal(t, expectedApplicationsEntry.Action.GetAction().ActionId, entries[0].Action.GetAction().ActionId, "PreQosPipe.applications action id does not equal expected",
			entries)
		require.Greater(t, uint8(maxApplicationIDs+1), uint8(entries[0].Action.GetAction().Params[0].Value[0]), "PreQosPipe.applications application id does not equal expected",
			entries)
		require.Less(t, uint8(0), uint8(entries[0].Action.GetAction().Params[0].Value[0]), "PreQosPipe.applications application id does not equal expected",
			entries)
	}
	...
	expected = buildExpectedSessionsDownlinkEntry(p4rtClient, expectedValues)
	...
	// check action param: session_meter_id, tunnelPeerID is randomly generated
	if afterModification {
		require.Greater(t, uint8(maxGTPTunnelPeerIDs+2), uint8(entries[0].Action.GetAction().Params[0].Value[0]), "PreQosPipe.sessions_downlink does not equal expected")
		require.Less(t, uint8(1), uint8(entries[0].Action.GetAction().Params[0].Value[0]), "PreQosPipe.sessions_downlink does not equal expected")
	}
	...
	// check ue_addr
	require.Equal(t, expected.GetMatch()[0], entries[0].GetMatch()[0], "PreQosPipe.terminations_uplink match fields do not equal expected")
	...
	require.NotEqual(t, []byte{0}, entries[0].Action.GetAction().Params[0].Value)
	// check UE address
	require.Equal(t, expected.GetMatch()[0], entries[0].GetMatch()[0], "PreQosPipe.terminations_downlink match fields do not equal expected")
	if afterModification {
	...
	if afterModification {
		...
		require.Greater(t, uint8(maxGTPTunnelPeerIDs+2), uint8(entries[0].GetMatch()[0].GetExact().Value[0]), "PreQosPipe.tunnel_peers does not equal expected")
		require.Less(t, uint8(1), uint8(entries[0].GetMatch()[0].GetExact().Value[0]), "PreQosPipe.tunnel_peers does not equal expected")

		expected = buildExpectedTunnelPeersEntry(p4rtClient, testdata)
		require.Equal(t, expected.Action.GetAction().ActionId, entries[0].Action.GetAction().ActionId, "PreQosPipe.tunnel_peers action ID does not equal expected")
	}
