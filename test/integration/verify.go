...
func buildExpectedTerminationsDownlinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expectedAppID uint8, afterModification bool) *p4_v1.TableEntry {
	...
	if afterModification {
		...
		qfi, _ := conversion.UInt32ToBinary(uint32(testdata.appQFI), 3)
...
func verifyP4RuntimeEntries(t *testing.T, testdata *pfcpSessionData, afterModification bool) {
	...
	//allocatedUplinkAppMeterIndex = binary.BigEndian.Uint16(entries[0].Action.GetAction().Params[1].Value)
	...
	require.Equal(t, 2, nrOfConfiguredMeters, "session meter should have 2 cells configured")
	...
	require.Equal(t, 2, nrOfConfiguredMeters, "app meter should have 2 cells configured")
}
