...
func buildExpectedSessionsDownlinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expectedTunnelPeerID uint8) *p4_v1.TableEntry {
	...
	}, client.NewTableActionDirect(ActSetDownlinkSession, [][]byte{tunnelPeerID, dummySessMeterIndex}), nil)
}

func buildExpectedTerminationsUplinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expectedAppID uint8, expectedTC uint8) *p4_v1.TableEntry {
	ueAddr, _ := conversion.IpToBinary(testdata.ueAddress)
...
func buildExpectedTerminationsDownlinkEntry(client *p4rtc.Client, testdata *pfcpSessionData,
	expectedAppID uint8, expectedTC uint8, afterModification bool) *p4_v1.TableEntry {
	ueAddr, _ := conversion.IpToBinary(testdata.ueAddress)
	appID, _ := conversion.UInt32ToBinary(uint32(expectedAppID), 3)
	...
	if afterModification {
		...
		if expectedTC != 0 {
			...
			actionParams = append(actionParams, []byte{expectedTC})
		}
...
func verifyP4RuntimeEntries(t *testing.T, testdata *pfcpSessionData, expectedValues p4RtValues, afterModification bool) {
	...
	expected = buildExpectedSessionsDownlinkEntry(p4rtClient, testdata, expectedTunnelPeerID)
	...
	expected = buildExpectedTerminationsUplinkEntry(p4rtClient, testdata, expectedValues.appID, expectedValues.tc)
	...
	expected = buildExpectedTerminationsDownlinkEntry(p4rtClient, testdata, expectedValues.appID, expectedValues.tc, afterModification)
	// we don't compare the entire object because counter ID is auto-generated by pfcpiface
...
func verifyNoP4RuntimeEntries(t *testing.T, expectedValues p4RtValues) {
	...
	require.Equal(t, expectedAllEntries, len(allInstalledEntries),
		fmt.Sprintf("UP4 should have only %d entry installed", expectedAllEntries))
