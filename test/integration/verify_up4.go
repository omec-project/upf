...
func buildExpectedInterfacesEntries(client *p4rtc.Client, testdata *pfcpSessionData, expectedValues TestExpectations) []*p4_v1.TableEntry {
...
func buildExpectedApplicationsEntry(client *p4rtc.Client, testdata *pfcpSessionData, expectedValues TestExpectations) *p4_v1.TableEntry {
...
func buildExpectedSessionsDownlinkEntry(client *p4rtc.Client, expected TestExpectations, ueState UEState) *p4_v1.TableEntry {
...
func buildExpectedTerminationsUplinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expected TestExpectations) *p4_v1.TableEntry {
	ueAddr, _ := conversion.IpToBinary(expected.ueAddress)
	dummyAppID := 0x00
	appID, _ := conversion.UInt32ToBinary(uint32(dummyAppID), 3)
...
func buildExpectedTerminationsDownlinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expected TestExpectations, afterModification bool) *p4_v1.TableEntry {
	ueAddr, _ := conversion.IpToBinary(expected.ueAddress)
	dummyAppID := 0x00
	appID, _ := conversion.UInt32ToBinary(uint32(dummyAppID), 3)
...
func verifyP4RuntimeEntries(t *testing.T, testdata *pfcpSessionData, expectedValues TestExpectations, ueState UEState) {
...
func verifyNoP4RuntimeEntries(t *testing.T, expectedValues TestExpectations) {
	p4rtClient, err := providers.ConnectP4rt("127.0.0.1:50001", false)
	require.NoErrorf(t, err, "failed to connect to P4Runtime server")
	defer providers.DisconnectP4rt()
...
func verifyP4RuntimeSliceMeter(t *testing.T, expectedValues TestExpectations) {
	p4rtClient, err := providers.ConnectP4rt("127.0.0.1:50001", false)
	require.NoErrorf(t, err, "failed to connect to P4Runtime server")
	defer providers.DisconnectP4rt()
